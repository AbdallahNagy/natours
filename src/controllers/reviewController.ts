...
export const createReview = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  if (req.user._id.toString() !== user) return next(new Error('You can only create reviews for yourself!'));
...
export const restrictToReviewAuthor = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction) => {
  const review = await Review.findById(req.params.id);

  if (!review) return next(new Error('No review found with that ID!'));

  const reviewUserId = (review.user as any)._id ?? review.user;
